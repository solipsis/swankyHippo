...
!function (e, t, i, n) {
  "use strict";
  t = "undefined" != typeof t && t.Math == Math ? t : "undefined" != typeof self && self.Math == Math ? self : Function("return this")(), e.fn.shape = function (a) {
    var o,
        r = e(this),
      ...
      var t,
          ...
          F = !1,
          H = this,
          T = C.data(w);m = { initialize: function initialize() {
          m.verbose("Initializing module for", H), m.set.defaultSide(), m.instantiate();
        }, instantiate: function instantiate() {
          m.verbose("Storing instance of module", m), T = m, C.data(w, T);
        }, destroy: function destroy() {
          m.verbose("Destroying previous module for", H), C.removeData(w).off(S);
        }, refresh: function refresh() {
          m.verbose("Refreshing selector cache for", H), C = e(H), z = e(this).find(b.shape), W = e(this).find(b.side);
        }, repaint: function repaint() {
          m.verbose("Forcing repaint event");var e = z[0] || i.createElement("div");e.offsetWidth;
          ...
          }, h.beforeChange.call(f[0]), m.get.transitionEvent() ? (m.verbose("Starting CSS animation"), C.addClass(y.animating), z.css(e).one(m.get.transitionEvent(), i), m.set.duration(h.duration), g(function () {
            C.addClass(y.animating), t.addClass(y.hidden);
          })) : i();
        }, queue: function queue(e) {
          m.debug("Queueing animation of", e), z.one(m.get.transitionEvent(), function () {
            m.debug("Executing queued animation"), setTimeout(function () {
              C.shape(e);
            }, 0);
          });
        }, reset: function reset() {
          m.verbose("Animating states reset"), C.removeClass(y.animating).attr("style", "").removeAttr("style"), z.attr("style", "").removeAttr("style"), W.attr("style", "").removeAttr("style").removeClass(y.hidden), f.removeClass(y.animating).attr("style", "").removeAttr("style");
            ...
            t = C.find("." + h.className.active), f = t.next(b.side).length > 0 ? t.next(b.side) : C.find(b.side).first(), F = !1, m.verbose("Active side set to", t), m.verbose("Next side set to", f);
          }, duration: function duration(e) {
            e = e || h.duration, e = "number" == typeof e ? e + "ms" : e, m.verbose("Setting animation duration", e), (h.duration || 0 === h.duration) && z.add(W).css({ "-webkit-transition-duration": e, "-moz-transition-duration": e, "-ms-transition-duration": e, "-o-transition-duration": e, "transition-duration": e });
            ...
            var e = C.clone().addClass(y.loading),
                ...
                i = F ? e.find(b.side).eq(F) : t.next(b.side).length > 0 ? t.next(b.side) : e.find(b.side).first(),
                n = "next" == h.width ? i.outerWidth(!0) : "initial" == h.width ? C.width() : h.width,
                a = "next" == h.height ? i.outerHeight(!0) : "initial" == h.height ? C.height() : h.height;t.removeClass(y.active), i.addClass(y.active), e.insertAfter(C), e.remove(), "auto" != h.width && (C.css("width", n + h.jitter), m.verbose("Specifying width during animation", n)), "auto" != h.height && (C.css("height", a + h.jitter), m.verbose("Specifying height during animation", a));
          }, nextSide: function nextSide(e) {
            F = e, f = W.filter(e), F = W.index(f), 0 === f.length && (m.set.defaultSide(), m.error(x.side)), m.verbose("Next side manually set to", f);
          }, active: function active() {
            m.verbose("Setting new side to active", f), W.removeClass(y.active), f.addClass(y.active), h.onChange.call(f[0]), m.set.defaultSide();
          } }, flip: { up: function up() {
            if (m.is.complete() && !m.is.animating() && !h.allowRepeats) return void m.debug("Side already visible", f);if (m.is.animating()) m.queue("flip up");else {
              m.debug("Flipping up", f);var e = m.get.transform.up();m.set.stageSize(), m.stage.above(), m.animate(e);
            }
          }, down: function down() {
            if (m.is.complete() && !m.is.animating() && !h.allowRepeats) return void m.debug("Side already visible", f);if (m.is.animating()) m.queue("flip down");else {
              m.debug("Flipping down", f);var e = m.get.transform.down();m.set.stageSize(), m.stage.below(), m.animate(e);
            }
          }, left: function left() {
            if (m.is.complete() && !m.is.animating() && !h.allowRepeats) return void m.debug("Side already visible", f);if (m.is.animating()) m.queue("flip left");else {
              m.debug("Flipping left", f);var e = m.get.transform.left();m.set.stageSize(), m.stage.left(), m.animate(e);
            }
          }, right: function right() {
            if (m.is.complete() && !m.is.animating() && !h.allowRepeats) return void m.debug("Side already visible", f);if (m.is.animating()) m.queue("flip right");else {
            ...
            var e = { active: t.outerWidth(!0), next: f.outerWidth(!0) },
                i = { origin: (e.active - e.next) / 2, depth: { active: e.next / 2, next: e.active / 2 } };m.verbose("Setting the initial animation position as left", f, i), z.css({ transform: "translateZ(-" + i.depth.active + "px)" }), t.css({ transform: "rotateY(0deg) translateZ(" + i.depth.active + "px)" }), f.addClass(y.animating).css({ left: i.origin + "px", transform: "rotateY(-90deg) translateZ(" + i.depth.next + "px)" });
          }, right: function right() {
            var e = { active: t.outerWidth(!0), next: f.outerWidth(!0) },
                i = { origin: (e.active - e.next) / 2, depth: { active: e.next / 2, next: e.active / 2 } };m.verbose("Setting the initial animation position as left", f, i), z.css({ transform: "translateZ(-" + i.depth.active + "px)" }), t.css({ transform: "rotateY(0deg) translateZ(" + i.depth.active + "px)" }), f.addClass(y.animating).css({ left: i.origin + "px", transform: "rotateY(90deg) translateZ(" + i.depth.next + "px)" });
          ...
          if (m.debug("Changing setting", t, i), e.isPlainObject(t)) e.extend(!0, h, t);else {
            if (i === n) return h[t];e.isPlainObject(h[t]) ? e.extend(!0, h[t], i) : h[t] = i;
          }
          ...
          h.silent || (m.error = Function.prototype.bind.call(console.error, console, h.name + ":"), m.error.apply(console, arguments));
            ...
            var t, i, n;h.performance && (t = new Date().getTime(), n = s || t, i = t - n, s = t, d.push({ Name: e[0], Arguments: [].slice.call(e, 1) || "", Element: H, "Execution Time": i })), clearTimeout(m.performance.timer), m.performance.timer = setTimeout(m.performance.display, 500);
          ...
          var r,
              s,
              d,
              l = T;return i = i || c, a = H || a, "string" == typeof t && l !== n && (t = t.split(/[\. ]/), r = t.length - 1, e.each(t, function (i, a) {
        ...
        } }, u ? (T === n && m.initialize(), m.invoke(l)) : (T !== n && T.invoke("destroy"), m.initialize());
    }), o !== n ? o : this;
