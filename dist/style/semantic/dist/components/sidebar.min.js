...
!function (e, i, n, t) {
  "use strict";
  i = "undefined" != typeof i && i.Math == Math ? i : "undefined" != typeof self && self.Math == Math ? self : Function("return this")(), e.fn.sidebar = function (o) {
    var r,
        s = e(this),
        a = e(i),
        l = e(n),
      ...
      var s,
          ...
          w.verbose("Destroying previous module for", H), H.off(P).removeData(E), w.is.ios() && w.remove.ios(), M.off(p), a.off(p), l.off(p);
        }, event: { clickaway: function clickaway(e) {
            var i = R.find(e.target).length > 0 || R.is(e.target),
                n = M.is(e.target);i && (w.verbose("User clicked on dimmed page"), w.hide()), n && (w.verbose("User clicked on dimmable context (scaled out page)"), w.hide());
            ...
            w.verbose("Adding clickaway events to context", M), T.closable && M.on("click" + p, w.event.clickaway).on("touchend" + p, w.event.clickaway);
            ...
            w.verbose("Removing clickaway events from context", M), M.off(p);
          }, scrollLock: function scrollLock() {
          ...
          w.verbose("Refreshing selector cache"), M = e(T.context), D = M.children(x.sidebar), R = M.children(x.pusher), j = M.children(x.fixed), w.clear.cache();
        }, refreshSidebars: function refreshSidebars() {
          w.verbose("Refreshing other sidebars"), D = M.children(x.sidebar);
          ...
          var n,
              ...
              _o,
              r = w.get.transition(),
              s = "overlay" === r || w.othersActive() ? H : R;i = e.isFunction(i) ? i : function () {}, "scale down" == T.transition && w.scrollToTop(), w.set.transition(r), w.repaint(), n = function n() {
            w.bind.clickaway(), w.add.inlineCSS(), w.set.animating(), w.set.visible();
          }, t = function t() {
            w.set.dimmed();
          }, _o = function o(e) {
            e.target == s[0] && (s.off(k + p, _o), w.remove.animating(), w.bind.scrollLock(), i.call(z));
          }, s.off(k + p), s.on(k + p, _o), v(n), T.dimPage && !w.othersVisible() && v(t);
        }, pullPage: function pullPage(i) {
          var n,
              _t,
              o = w.get.transition(),
              r = "overlay" == o || w.othersActive() ? H : R;i = e.isFunction(i) ? i : function () {}, w.verbose("Removing context push state", w.get.direction()), w.unbind.clickaway(), w.unbind.scrollLock(), n = function n() {
            w.set.transition(o), w.set.animating(), w.remove.visible(), T.dimPage && !w.othersVisible() && R.removeClass(S.dimmed);
          }, _t = function t(e) {
            e.target == r[0] && (r.off(k + p, _t), w.remove.animating(), w.remove.transition(), w.remove.inlineCSS(), ("scale down" == o || T.returnScroll && w.is.mobile()) && w.scrollBack(), i.call(z));
          }, r.off(k + p), r.on(k + p, _t), v(n);
          ...
          if (w.debug("Changing setting", i, n), e.isPlainObject(i)) e.extend(!0, T, i);else {
            if (n === t) return T[i];e.isPlainObject(T[i]) ? e.extend(!0, T[i], n) : T[i] = n;
          }
          ...
          !T.silent && T.debug && (T.performance ? w.performance.log(arguments) : (w.debug = Function.prototype.bind.call(console.info, console, T.name + ":"), w.debug.apply(console, arguments)));
        }, verbose: function verbose() {
          !T.silent && T.verbose && T.debug && (T.performance ? w.performance.log(arguments) : (w.verbose = Function.prototype.bind.call(console.info, console, T.name + ":"), w.verbose.apply(console, arguments)));
        }, error: function error() {
          T.silent || (w.error = Function.prototype.bind.call(console.error, console, T.name + ":"), w.error.apply(console, arguments));
